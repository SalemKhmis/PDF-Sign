...
// NEW: Function to get PDF bytes without downloading
export async function getPDFBytes(pdfFile, objects, name) {
  const PDFLib = await getAsset('PDFLib');
  const makeTextPDF = await getAsset('makeTextPDF');
  let pdfDoc;
  try {
  ...
  try {
    const pdfBytes = await pdfDoc.save();
    return pdfBytes; // Returns Uint8Array
  } catch (e) {
    console.log('Failed to save PDF.');
    throw e;
  }
...
// NEW: Function to get PDF as Blob
export async function getPDFAsBlob(pdfFile, objects, name) {
  const pdfBytes = await getPDFBytes(pdfFile, objects, name);
  return new Blob([pdfBytes], { type: 'application/pdf' });
}

// NEW: Function to save AND get PDF bytes
export async function saveAndGetPDF(pdfFile, objects, name) {
  const PDFLib = await getAsset('PDFLib');
  const download = await getAsset('download');
  const makeTextPDF = await getAsset('makeTextPDF');
  let pdfDoc;
